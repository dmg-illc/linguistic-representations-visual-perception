...
# it's weird that there's no processor, but it seems to be like this on HF
...
for batch in batchify(img_file_names, batch_size):
    ...
    inputs = feature_extractor(images=images, return_tensors="pt").to(model.device)
    output = model(
                    **inputs,
...
(ROOT / f'results/image_features/vit').mkdir(parents=True, exist_ok=True)

save_pickle(mean_ftr_dict_dict, ROOT / f'results/image_features/vit/vit.pkl')