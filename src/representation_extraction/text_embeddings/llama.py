...
from src.format_sentences import format_input_sentences, batchify
import torch
import numpy as np


parser = argparse.ArgumentParser()
parser.add_argument('-c', '--captions', choices=['llava-ov', 'molmo', 'phi-4', 'pixtral', 'qwen-vl', 'coco']) 
...
(ROOT / f'results/caption_embeddings/llama3').mkdir(parents=True, exist_ok=True)

save_pickle(last_tok_dict, ROOT / f'results/caption_embeddings/llama3/llama3_{args.captions}_last.pkl')
save_pickle(mean_tok_dict, ROOT / f'results/caption_embeddings/llama3/llama3_{args.captions}_mean.pkl')
